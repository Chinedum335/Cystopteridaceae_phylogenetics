...
output_folder <- "../output"  # Add output folder name

# Update paths to use the specified output folder
genecopyFn <- 'subgenomes_names.csv'
tree_file <- file.path(output_folder, 'homologizer_map.tree')
input_dir <- file.path(output_folder, '/')
prefix <- file.path(output_folder, 'homologizer')
                        ...
                        legend_title = "value")
{
  ...
  if (is.null(color)) {
    ...
    p2 <- p2 + geom_text(data = dd2, aes(x, y, label=value), size=1, fontface = "bold", inherit.aes = FALSE)
...
p = ggtree(tree) 
p = p + geom_tiplab(size = 2, fontface = "bold", align = TRUE, linesize = 0.25, offset = 0.0005)
...
# Save files with dynamic output folder
pdf_file <- paste0("Plot.pdf")
csv_file <- paste0("joint_map_phase_results.csv")

ggsave(pdf_file, height = 9, width = 10)
# write.csv(joint_map_phase_results, file = csv_file)
